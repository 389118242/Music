			...
			.ajax({
				...
				success : function(ret) {
					...
					for (var i = 0; i < list.length; i++) {
						var ii = list[i];
						if (ii.user.userImg == null) {
							ii.user.userImg = "images/user_default.png";
						}
						var str = '<article id="comment-id-1" class="comment-item">'
						...
						$parent.append(str);
						if (ii.parent !== null) {
							if (ii.parent.user.userImg == null) {
								ii.parent.user.userImg = "images/user_default.png";
							}
							var str = '<article class="comment-item comment-reply">'
									+ '<a class="pull-left thumb-sm"><img src="'
									+ ii.parent.user.userImg
									+ '" class="img-circle">'
									+ '</a><section class="comment-body m-b"><header>'
									+ '<a href="profile?userId='
									+ ii.parent.user.userId
									+ '"><strong>'
									+ ii.parent.user.userName
									+ '</strong></a>'
									+ '<label class="label bg-dark m-l-xs"></label>'
									+ '<span class="text-muted text-xs block m-t-xs">&nbsp;</span></header>'
									+ '<div class="m-t-sm">'
									+ ii.parent.comContent
									+ '</div>'
									+ '</section>' + '</article>';
							$parent.append(str);
						}
...
$('button[type=submit][class="abtn btn-success"]').bind({
	click : function(event) {
		...
		console.log(JSON.stringify($('#temp').html()));
