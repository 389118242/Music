					...
					<div class="col-sm-8">
						<div class="panel wrapper-lg">
							<div class="row">
								...
								<div class="col-sm-7">
									<h4 class="m-t-none text-black">${songList.listName}</h4>
									<div class="clearfix m-b-lg">
										<div class="clear">
											<a class="text-muted" href="profile?userId=${user.userId}"
												style="padding: 15px 0px">By ${user.userName}</a> <small
												class="block text-muted" style="padding-top: 15px">${songList.collectionNum}
												收藏 / ${songList.playNum} 播放</small>
										</div>
									</div>
									<div class="m-b-lg">
										<c:if test="${fn:length(songList.songsList)>0 }">
											<a href="#" id="${songList.listId }" class="btn btn-sm btn-icon btn-info addMusicListToListAndPlay"><i
												class=" icon-control-play"></i></a>
											<a href="#" id="${songList.listId }"
												class="btn btn-rounded btn-sm btn-icon btn-default addMusicListToList"><i
												class="icon-plus"></i></a>
										</c:if>
										<c:if
											test="${null!=sessionScope.user&&sessionScope.user.userId!=songList.userId}">
											<a href="javascript:;"
												class="btn btn-rounded btn-sm btn-icon btn-default"><i
												id="collectionSL"
												<c:if test="${null==collection }">
													class="icon-heart"
												</c:if>
												<c:if test="${null!=collection }">
													class="fa fa-heart"
										...
										<c:if
											test="${null!=sessionScope.user&&sessionScope.user.userId==songList.userId}">
											<a href="editSongList?songListId=${songList.listId }"
												class="btn btn-rounded btn-sm btn-icon btn-default"><i
												class="icon-note"></i></a>
										</c:if>
									...
									<c:if test="${fn:length(listKind)>0 }">
										<div>
											标签:
											<c:forEach items="${listKind }" var="item">
												<a href="javascript:;" class="badge bg-light">${item.kindName }</a>
											</c:forEach>
										</div>
									</c:if>
								</div>
							...
							<div class="m-t">
								<p>介绍：${songList.listDetail}</p>
							</div>
							<h4 class="m-t-lg m-b">歌曲列表</h4>
							<ul class="list-group list-group-lg">
								<c:if test="${songList.songsList!=null}">
									<c:forEach items="${songList.songsList}" var="song">
										<li class="list-group-item" id="${song.songId}"
											value="${song.songId}">
											<div class="pull-right m-l">
												<a href="#" class="m-r-sm"><i class="icon-plus"></i></a> <a
													name="collect" href="#" class="m-r-sm"><i
